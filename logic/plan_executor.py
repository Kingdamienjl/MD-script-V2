...
class PlanExecutor:
    ...
    def execute_next(self, actions: list[Action], index: int, client: object) -> bool:
        if index < 0 or index >= len(actions):
            return False
        return self._execute_with_retry(client, actions[index])

    def _execute_with_retry(self, client: object, action: Action) -> bool:
