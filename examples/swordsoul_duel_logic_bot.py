...
class SwordsoulDuelLogicBot:
    ...
    def handle_my_main_phase_1(self) -> None:
        ...
        ctx = build_context(self, self.profile_index, snapshot)
        ctx["ruleset"] = self.ruleset
        ctx["turn_cooldowns"] = self.turn_cooldowns
        ctx["actions_this_turn"] = self.actions_this_turn
        ...
        result = self.plan_executor.execute(self, actions, ctx)
        self.actions_this_turn = ctx.get("actions_this_turn", self.actions_this_turn)
        if result.bailed_out:
            return

    def _merge_hand_names(self, ctx: dict, snapshot) -> list[str]:
        hand_names = [card.name for card in snapshot.hand if card.name]
        if hand_names:
            return hand_names
        return list(ctx.get("hand_names", []))
